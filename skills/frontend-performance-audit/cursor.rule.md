# Frontend Performance Audit (Cursor rule)
scope: project
version: 0.1.0
...
- Reads lighthouse.config.json from the project root if present; falls back to defaults: urls=["http://localhost:3000"], buildCommand=npm run build, startCommand=npm run start, port=3000, categories=[performance, accessibility, best-practices], thresholds={performance: 100, accessibility: 100, best-practices: 90}.
...
- Installs Lighthouse CLI globally if not already installed, then runs it with --headless Chrome against each URL in the urls array. Stores timestamped JSON and HTML reports under lighthouse-reports/.
- Parses category scores from each URL's Lighthouse JSON output. Top-level scores in scores.json are the worst-case per category across all URLs. passed: true only if every URL meets every threshold.
- If all thresholds pass: writes lighthouse-reports/scores.json and commits it with message 'perf: lighthouse scores - perf:<score> a11y:<score> (N pages)'. Reports passing scores and stops.
- If any threshold fails: identifies failing audits (score !== null && score < 1) across all URLs, applies targeted fixes, rebuilds, and reruns all URLs. Repeats up to 3 iterations.
...
# metadata
id: charlie.frontend-performance-audit