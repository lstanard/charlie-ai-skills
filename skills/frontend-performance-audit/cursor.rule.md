...
- Reads lighthouse.config.json from the project root if present; falls back to defaults: url=http://localhost:3000, buildCommand=npm run build, startCommand=npm run start, port=3000, categories=[performance, accessibility], thresholds={performance: 100, accessibility: 100}.
