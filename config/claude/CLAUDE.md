...
* Acceptance criteria and checklist items are sentences too. Items like "Invariant assertion passes on the prod export" or "Dry-run diff log reviewed" delete the actor and the action, leaving only artifact names in passive fragments. Write what someone does or observes: "Run the script in dry-run mode against the prod export and confirm no row has `active` inconsistent with `status`." If an item names an artifact ("the diff log", "the invariant assertion") that the document hasn't described, describe the thing instead of naming it.
...
* Define at first use. Coined category labels, non-obvious acronyms, and shorthand system aliases get a few words of identification the first time they appear. Ticket/issue keys get a short parenthetical saying what they are. Skip this for terms the audience uses daily.
    ...
    * Draft-local numbering ("ticket 3", "PR 2", "option B") is a coined label too. It only means something inside the draft that defines it. Before content leaves the workspace (filed Jira tickets, published docs, PR descriptions), resolve every such reference to the real identifier and link it (the Jira key, the PR URL). For Jira dependencies, also set the actual issue-link relationship ("is blocked by"), not just prose.
